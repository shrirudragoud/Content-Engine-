...
const prompt = ai.definePrompt({
  ...
  output: {schema: GenerateAnimationCodeOutputSchema},
...
Image to use (embedded in the prompt): {{media url=imageDataUri}}
...
4.  **Crucially, the provided image MUST be embedded directly into the HTML using an \`<img>\` tag with its \`src\` attribute set to the image's base64 data URI (which is "{{imageDataUri}}").** Do not try to link to an external file for the image.
...
-   Self-contained: No external libraries or assets (except the embedded image).
...
<body>
    <div class="animation-container">
        <img src="{{{imageDataUri}}}" alt="Animated Content for {{{moduleTitle}}}" class="animated-image" />
...
Ensure the image \`src\` is exactly "{{{imageDataUri}}}".
...
const generateAnimationCodeFlow = ai.defineFlow(
  {
    ...
    outputSchema: GenerateAnimationCodeOutputSchema,
  },
  async (input) => {
    ...
    // Ensure the output has the htmlContent field
    if (typeof output.htmlContent !== 'string') {
        throw new Error('Generated output does not contain htmlContent string.');
    }
    return output;
  }
);
